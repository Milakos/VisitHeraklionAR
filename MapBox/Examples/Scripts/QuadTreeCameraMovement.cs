namespace Mapbox.Examples
{
    ...
    public class QuadTreeCameraMovement : MonoBehaviour
	{
		...
		public void Update()
		{
			...
			float currentZoom = _mapManager.Zoom;
			float newZoom = Mathf.Clamp(currentZoom, 5.5f, 17.5f);
		...
		private void LateUpdate()
		{
			...
			// {
				...
				{					
					if(UtilsHomeBar.fullScreenCheck == false)
					{
						HandleTouch();
					} 
		...
		{
			if(InputManager.Instance.zoom == false)
			{
				if (_useDegreeMethod)
				{
					UseDegreeConversion();
		...
		void UseDegreeConversion()
		{
			...
			if (_shouldDrag == true)
			{
				...
				if (Mathf.Abs(changeFromPreviousPosition.x) > 0.0f || Mathf.Abs(changeFromPreviousPosition.y) > 0.0f)
				{
					...
					var latitude = _mapManager.CenterLatitudeLongitude.x + offset.z * factor;
                    var longitude = _mapManager.CenterLatitudeLongitude.y + offset.x * factor;

                    // Check if the new position exceeds the bounds
                    // longitude = Mathf.Clamp((float)longitude, 25.106617f, 25.155660f);
                    // latitude = Mathf.Clamp((float)latitude, 35.301574f, 35.349800f);
